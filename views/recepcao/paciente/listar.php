...
<table class="table table-sm mt-3">
    ...
    $caminho_pag = 'index.php?menu='.$pagina.'&';
    ...
    for ($i=0; $i < count($dados); $i++) { 
        ...
        $id = $dados[$i]['id']; 
        $nsus = $dados[$i]['num_sus'];
        $nome = $dados[$i]['nome'];
        $cpf = $dados[$i]['cpf'];
        $telefone = $dados[$i]['telefone'];
        $idade = $dados[$i]['idade'];
        $transporte = $dados[$i]['transporte'];
...
echo '
        <tr>
            ...
            <td>'.$idade.'</td>
            <td>
                <a href="index.php?menu='.$pagina.'&funcao=editar&id='.$id.'"><i class="fas fa-edit text-info"></i></a>
                <a href="index.php?menu='.$pagina.'&funcao=excluir&id='.$id.'"><i class="far fa-trash-alt text-danger"></i></a>
            </td>
        </tr>';
    }
...
if ($txtbuscar == '') {
    echo '
    <!--ÁREA DA PÁGINAÇÃO -->
    <nav class="paginmenu" aria-label="Page navigation example">
